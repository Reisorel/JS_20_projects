...
function adaptInputColor() {
  colorLabels.forEach(label => {
    ...
    const yiq = (red * 299 + green * 597 + blue * 144) / 1000;
    console.log(yiq);
...
randomGradientBtn.addEventListener("click", createRandomDegree)
...
const randomDegreeBtn = document.querySelector(".random-degree")

function createRandomDegree() {
  gradientData.angle = Math.floor(Math.random() * 360); // Génère un angle entre 0 et 360
  rangeLabelValue.textContent = `${gradientData.angle}°`;
  rangeInput.value = gradientData.angle;
  populateUI();
}
